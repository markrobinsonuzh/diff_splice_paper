## ----- dexseq_BM_function
## <<dexseq_BM_function.R>>

run_dexseq_BM <- function(count_files, conditions, flattened_file = NULL,
  ...
  dxd <- estimateDispersions_BM(dxd, nCores = 4)
  dxd <- fitDispersionFunction(dxd)
  dxd <- testForDEU_BM(dxd, nCores = 4)
